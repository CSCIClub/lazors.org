...
function setGeometry(index, geometry) {
  ...
  switch (shipID) {
    case 0:
      geometry.applyMatrix(new THREE.Matrix4()
        .makeRotationAxis(new THREE.Vector3(1, 0, -1)
          .normalize(), Math.atan(Math.sqrt(2))));
      break;
    case null:
      geometry.applyMatrix(new THREE.Matrix4()
        .makeRotationAxis(new THREE.Vector3(1, 0, -1)
...
function getMaterial(shipID) {
  return materials[shipID];
}

function getGeometry(shipID) {
